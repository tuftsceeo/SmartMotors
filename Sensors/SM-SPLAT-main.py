...
def receive():
    ...
    for mac, message, rtime in networking.aen.return_messages(): #You can directly iterate over the function        
        if(mac == broadcast_mac):
            print("received")
            #print("the message is", message, struct.unpack("I",message)[0])
            _sensor_value = (~message & MASK4)
            if (_sensor_value >> 3 & 0b1):
                sensor_value = 4000
    
            if (_sensor_value >> 2 & 0b1):
                sensor_value = 3000

            if (_sensor_value >> 1 & 0b1):
                sensor_value = 2000

            if (_sensor_value & 0b1):
                sensor_value = 1000
...
while True:
    ...
    light, pot = sens.readpoint()
    point = (sensor_value, pot)
    if(triggered):
        if LOGGING:
            savetolog(time.time(),screenID,highlightedIcon, point,points)
        triggered=False
